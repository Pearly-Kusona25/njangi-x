...
serve(async (req: Request) => {
  ...
  const group_id = query.get("group_id") ?? (await req.json().then((body: any) => body?.group_id).catch(() => null));
  ...
  const ordered = members as Array<{ payout_position: number; user_id: string; users: any }>;
